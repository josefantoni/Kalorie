...
function searchTerms(input) {
  ...
  const terms = new Set();
  for (const word of folded.split(/\s+/).filter((w) => w.length > 0)) {
...
async function main() {
  ...
  if (!args.key) {
    console.error('Usage: node backfill-search-terms.js --key <path-to-service-account.json> [--dry-run]');
    process.exit(1);
  }
  ...
  let alreadyHasTerms = 0;

  for (const doc of snapshot.docs) {
    const data = doc.data();
    if (Array.isArray(data.cz_name_search_terms) && Array.isArray(data.eng_name_search_terms)) {
      alreadyHasTerms++;
      continue;
    }
  ...
  console.log(`Already has search terms: ${alreadyHasTerms}`);
  ...
  for (let i = 0; i < toUpdate.length; i += args.batchSize) {
    ...
    for (const item of chunk) {
      batch.update(db.collection('foodItems').doc(item.id), {
        cz_name_search_terms: item.cz_name_search_terms,
        eng_name_search_terms: item.eng_name_search_terms,
      });
    }
    await batch.commit();
