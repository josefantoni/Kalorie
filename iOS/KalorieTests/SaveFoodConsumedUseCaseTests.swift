...
final class SaveFoodConsumedUseCaseTests: XCTestCase {
    ...
    func test_saveFoodConsumed_whenNotAuthenticated_throwsAuthError() async throws {
        ...
        do {
            try await sut(makeItem(), grams: 100, date: .now, mealTypeId: nil)
    ...
    func test_saveFoodConsumed_calculatesCaloriesFromWeightAndCaloriesPer100g() async throws {
        let (sut, dataProvider) = makeSUT()
        try await sut(makeItem(caloriesPerHundredGrams: 100), grams: 200, date: .now, mealTypeId: nil)
        XCTAssertEqual(dataProvider.savedDTO?.calories, 200)
    }

    func test_savesRoundedCalories_whenScalingProducesFraction() async throws {
        let (sut, dataProvider) = makeSUT()
        try await sut(makeItem(caloriesPerHundredGrams: 133), grams: 150, date: .now, mealTypeId: nil)
        XCTAssertEqual(dataProvider.savedDTO?.calories, 200)
    }

    func test_savesRoundedCalories_whenCaloriesPerHundredGramsIsFractional_roundsOnce() async throws {
        let (sut, dataProvider) = makeSUT()
        try await sut(makeItem(caloriesPerHundredGrams: 133.6), grams: 150, date: .now, mealTypeId: nil)
        XCTAssertEqual(dataProvider.savedDTO?.calories, 200)
    }
    ...
    func test_saveFoodConsumed_storesCorrectNameAndWeight() async throws {
        let (sut, dataProvider) = makeSUT()
        try await sut(makeItem(czName: "Tvaroh", engName: "Cottage cheese"), grams: 150, date: .now, mealTypeId: nil)
    ...
    func test_saveFoodConsumed_scalesEnergyKJFatSaturatedAndFiberToLoggedWeight() async throws {
        let (sut, dataProvider) = makeSUT()
        try await sut(makeItem(fiber: 3), grams: 200, date: .now, mealTypeId: nil)
    ...
    func test_saveFoodConsumed_storesTheProvidedMealTypeId() async throws {
        let (sut, dataProvider) = makeSUT()
        try await sut(makeItem(), grams: 100, date: .now, mealTypeId: "lunch")
        XCTAssertEqual(dataProvider.savedDTO?.mealTypeId, "lunch", "the use case writes whatever pin the caller resolved; it no longer resolves a window itself")
    }

    func test_saveFoodConsumed_whenMealTypeIdIsNil_storesNoPin() async throws {
        let (sut, dataProvider) = makeSUT()
        try await sut(makeItem(), grams: 100, date: .now, mealTypeId: nil)
        XCTAssertNil(dataProvider.savedDTO?.mealTypeId, "a caller that resolved no window must be able to write no pin")
    }
